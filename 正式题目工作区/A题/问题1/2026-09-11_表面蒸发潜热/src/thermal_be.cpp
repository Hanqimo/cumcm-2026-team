// M02-v01: thermal response to the unchanged moisture solution.
...
int main(int argc,char**argv){try{
 ...
 constexpr double rhoD=820/3.55,lvA=2500900,lvB=2370;Grid q(n);V T(n+1,28),T0=T,mu(n+1),g(n),zero(n+1);Linear mat(n),mat0(n);
 ...
 double t=0,prevdt=-1,maxlinear=0,maxres=0,maxheat=0,minT=28,maxT=28,minTime=0,minRad=0,maxexact=0,minhe=H;
 ...
 double dt=std::min({scheduled(t,base,1),end-t,std::floor(t+1e-10)+1-t});double tn=t+dt;
 ...
 if(dt!=prevdt){for(int i=0;i<=n;i++)mu[i]=B*q.w[i]/dt;mat.factor(g,mu,H);mat0.factor(g,mu,H);prevdt=dt;}
 // Only the last elimination pivot changes with j; all preceding pivots are unchanged.
 mat.diag[n]=mu[n]+g[n-1]+R*he;double pivot=mat.diag[n]+g[n-1]*mat.cp[n-1];if(!(pivot>0))throw std::runtime_error("invalid thermal pivot");mat.inv[n]=1/pivot;
 mat.solve(g,mu,T,he,forcing/he,zero);mat0.solve(g,mu,T0,H,te,zero);
 maxlinear=std::max({maxlinear,mat.linear_error(g),mat0.linear_error(g)});
 ...
 for(int i=0;i<=n;i++){
 double u=mat.out[i];if(!std::isfinite(u)||u<low-1e-8||u>high+1e-8)throw std::runtime_error("temperature comparison bound");
 if(mode=="real"&&(u<0||u>50))throw std::runtime_error("temperature outside liquid-water property range [0,50] C");
 energy+=(long double)B*q.w[i]*(u-T[i]);double residual=mu[i]*(u-T[i]);if(i)residual+=g[i-1]*(u-mat.out[i-1]);if(i<n)residual-=g[i]*(mat.out[i+1]-u);else residual-=R*(H*(te-u)-lambda*(lvA-lvB*u)*j);
 maxres=std::max(maxres,std::abs(residual)/mat.diag[i]);
 if(u<minT){minT=u;minTime=tn;minRad=q.r[i];}maxT=std::max(maxT,u);
 }
 long double hc=(long double)dt*R*H*(te-mat.out[n]),hl=(long double)dt*R*lambda*(lvA-lvB*mat.out[n])*j;long double b=energy-hc+hl;
 maxheat=std::max(maxheat,std::abs(double(b)));balance+=b;convTotal+=hc;latentTotal+=hl;massFromInterpolated+=(long double)dt*R*j;
