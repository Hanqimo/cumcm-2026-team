...
def main():
    ...
    for seed,targets in fixtures.items():
        for name in variants:
            ...
            try:
                factory=ConvexCoverPlanner if name=='baseline' else CostAwarePlanner
                params=dict(base,**config.get(name,{}))
                kind=params.pop('_class',None)
                if kind=='merge':factory=MergedCoverPlanner
                if kind=='combined':factory=CombinedCostCoverPlanner
            ...
            rows.append(dict(seed=seed,variant=name,n=len(targets),cleared=len(world.cleared),
                ...
                region_checks=world.region_checks,clear_checks=world.clear_checks,compute_s=time.perf_counter()-begin))
