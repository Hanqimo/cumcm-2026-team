...
def main():
    ...
    for p in [*list((ROOT/'src').glob('*.py')),Path(__file__),Path(__file__).with_name('trace_world.py'),physical,Path(__file__).with_name('geometry_check.py')]:shutil.copy2(p,snap/p.name)
    ...
    for seed,targets in fixtures.items():
        for name in variants:
            ...
            try:
                ...
                if kind=='action':factory=ActionValuePlanner
                if factory in [ServicePlanner,TransitSearchPlanner,LookaheadPlanner,MilpPlanner,ActionValuePlanner]:
