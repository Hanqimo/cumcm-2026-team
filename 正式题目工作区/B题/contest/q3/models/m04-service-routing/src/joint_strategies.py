...
class JointPlanner(BasePlanner):
    def __init__(self, robot, *, family='joint', scan_gain=.05, info_distance=150.,
                 ...
                 joint_negatives=False,shared_waypoint=False,explore_bonus=0.,expected_count=13.,coverage_value=False,coverage_value_threshold_s=10.,motion='side',first_fraction=1.,radius_coupling=False):
        ...
        self.motion,self.first_fraction=motion,first_fraction
        self.radius_coupling=radius_coupling
        if radius_coupling:self.negative_regions=True
    ...
    def solve_track(self,track):
        if self.motion=='midpoint':
            for _ in range(16):
                self.robot.check_budget()
                if track.radius<=CLEAR_RADIUS:return super().solve_track(track)
                q=track.center.copy()
                if len(track.history)==1:
                    s=np.asarray(track.history[0]['position'])
                    q=s+self.first_fraction*(q-s)
                if any(math.dist(q,h['position'])<.1 for h in track.history):
                    q=track.next_point(np.asarray(self.robot.position))
                if self.observe(q,track.channel)['measure_result']=='no_signal':
                    raise ArithmeticError('Midpoint reception certificate failed')
                if self.one_step:return
            raise RuntimeError('Midpoint localization exhausted budget')
    ...
    def observe(self, point, channel):
        if not self.negative_regions:
            ...
            if channel in self.tracks:
                ...
                if self.radius_coupling:
                    for positive in track.history:
                        s=np.asarray(positive['position'])
                        for negative in self.coverage.samples[channel]:
                            n=np.asarray(negative)
                            normal=2*(n-s)
                            offset=float(np.dot(n,n)-np.dot(s,s))+1e-6*np.linalg.norm(normal)
                            track.poly=clip_halfplanes(track.poly,[normal],[offset])
                    if not len(track.poly):raise ArithmeticError('Fixed-radius positive/negative conflict')
...
class PathCoverPlanner(JointPlanner):
    """Choose sensing stops on route segments; update insertion costs jointly."""
    def __init__(self,robot,*,path_spacing=200.,**kwargs):
        super().__init__(robot,family='adaptive',**kwargs)
        self.path_spacing=path_spacing
        self.path_static=None

    def adaptive_cover(self,nodes,unknown):
        def bits(mask):return int.from_bytes(np.packbits(mask,bitorder='little').tobytes(),'little')
        if self.path_static is None:
            pts=[np.asarray(p) for p in self.anchors]
            pts += [np.array([x,y]) for x in np.arange(-1500,1501,self.grid_step) for y in np.arange(-1500,1501,self.grid_step) if math.hypot(x,y)<1750]
            self.path_static=[(q,bits(self.coverage.mask(q))) for q in pts]
        covered=bits(self.coverage.covered[unknown[0]])
        full=(1<<len(self.coverage.centers))-1
        selected=[];self.cover_targets=set()
        p=np.asarray(self.robot.position)
        while covered!=full:
            future=nodes+[('scan',-100-i,q) for i,(q,mask) in enumerate(selected)]
            order=open_route(p,[n[2] for n in future])
            route=[p]+[future[i][2] for i in order]
            candidates=self.path_static.copy()
            for a,b in zip(route,route[1:]):
                count=max(1,int(math.ceil(np.linalg.norm(b-a)/self.path_spacing)))
                for t in np.linspace(0.,1.,count+1):
                    q=a+t*(b-a);candidates.append((q,bits(self.coverage.mask(q))))
            if len(route)==1:candidates.append((p,bits(self.coverage.mask(p))))
            def detour(q):
                best=float(np.linalg.norm(q-route[-1]))
                for a,b in zip(route,route[1:]):
                    best=min(best,float(np.linalg.norm(a-q)+np.linalg.norm(b-q)-np.linalg.norm(a-b)))
                return max(0.,best)
            missing=full^covered
            best=max(candidates,key=lambda a:(a[1]&missing).bit_count()/(6*len(unknown)+self.cover_move_weight*detour(a[0])/5))
            if not best[1]&missing:raise RuntimeError('Path cover cannot progress')
            covered|=best[1];selected.append(best)
        # Reverse-delete coverage redundancy before routing the chosen stops.
        for a in list(reversed(selected)):
            total=bits(self.coverage.covered[unknown[0]])
            for other in selected:
                if other is not a:total|=other[1]
            if total==full:selected=[x for x in selected if x is not a]
        return nodes+[('scan',-100-i,q) for i,(q,mask) in enumerate(selected)]
    ...
    def candidates(self):
        ...
        first=nodes[order[0]]
        if first[0]=='target':
            matching=[n for n in nodes if n[0]=='scan' and math.dist(n[2],first[2])<1e-5]
