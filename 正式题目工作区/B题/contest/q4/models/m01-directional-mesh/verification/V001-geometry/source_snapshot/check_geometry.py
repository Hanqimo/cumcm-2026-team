...
def main():
    ...
    # Optical cover is checked as a GEOS union of rotated closed squares, not
    # using the clipping routine that selected the cells.
    optical=[]
    for angle in [0.,43.,179.99,270.]:
        ...
        pair=max(((i,j) for i in range(len(p)) for j in range(len(p))),key=lambda ij:math.dist(p[ij[0]],p[ij[1]]))
        v=p[pair[1]]-p[pair[0]];v/=np.linalg.norm(v)
        w=np.array([-v[1],v[0]])
        squares=[Polygon([q+13*(a*v+b*w) for a,b in [(-1,-1),(1,-1),(1,1),(-1,1)]]) for q in cells]
        difference=Polygon(p).difference(unary_union(squares)).area
