...
VERSION = 'p4-adaptive-cover-v2-research'
from adaptive_cover import AdaptiveMesh
...
class Planner:
    def __init__(self, robot, policy='directional_v1'):
        ...
        self.mesh = (AdaptiveMesh(self.parameters['mesh_spacing_m'],self.parameters.get('cell_depth',2),self.parameters.get('layout'))
                     if self.parameters.get('adaptive_cover') else DirectionalMesh(self.parameters['mesh_spacing_m']))
        self.tracks = {}
        self.observations = {c:[] for c in range(1,21)}
        self.stop_reason = None
        self.optical_fallbacks = 0
        self.radio_losses = 0
        self.offered_channels=set()
    ...
    def solve_track(self, track):
        self._solve_track(track)
        if self.parameters.get("clear_scans") and track.channel in self.robot.cleared and self.unknown():
            q=np.asarray(self.robot.position);index=self.mesh.add_point(q)
            self.scan(index)

    def _solve_track(self, track):
    ...
    def run(self):
        ...
        for _ in range(1000):
            ...
            if isinstance(self.mesh,AdaptiveMesh):
                self.mesh.position=np.asarray(self.robot.position)
                if self.parameters.get('target_offers'):
                    for c in active:
                        t=self.tracks[c]
                        if c not in self.offered_channels:
                            self.mesh.add_point(t.center);self.offered_channels.add(c)
    ...
    def certificate(self):
        ...
        extra={}
        if isinstance(self.mesh,AdaptiveMesh):
            extra=dict(coverage_cells=[c.tolist() for c in self.mesh.cells],
                       cell_certificates={str(c):self.mesh.cell_certificate(c) for c in absent})
        return dict(**extra,problem=4,reason=self.stop_reason,complete=bool(self.stop_reason),
