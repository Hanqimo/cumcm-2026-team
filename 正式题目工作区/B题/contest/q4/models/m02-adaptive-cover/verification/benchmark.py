...
def fixture(seed, stress=None, profile=None):
    ...
    if stress=='near_cluster':
        ...
        reception[:]=1000.
    if profile=='mostly_directional':flags[:]=True;flags[-1]=False
    if profile=='mostly_omni':flags[:]=False;flags[0]=True
    ...
    return dict(seed=seed,sources=sources,stress=stress,profile=profile)
...
def main():
    ...
    p.add_argument('--profile',choices=['mostly_directional','mostly_omni'])
    args=p.parse_args();out=args.output;out.mkdir(parents=True,exist_ok=False)
    snap=out/'source_snapshot';snap.mkdir();hashes={}
    for f in list((MODEL/'src').glob('*.py'))+list((MODEL/'src').glob('*.json'))+[Path(__file__)]:
    ...
    for seed in range(args.start,args.start+args.count):
        data=fixture(seed,args.stress,args.profile)
        (out/f'fixture-{seed}.json').write_text(json.dumps(data,indent=2),encoding='utf-8')
        for policy in args.policies.split(','):
            world=World(data,args.error);t0=time.perf_counter();planner=Planner(world,policy);error=None
            try:planner.run()
