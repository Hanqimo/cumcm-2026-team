...
class FlexibleCoverPlanner(JointPlanner):
    ...
    def candidates(self):
        ...
        for c,t in self.tracks.items():
            ...
            if q is None:
                q=t.next_point(np.asarray(self.robot.position)) if getattr(self,'cover_target_entry',False) else self.target_position(t)
        ...
        def total_without(i):
            ...
            return total
        def exact_union_without(i):
            sites=list(self.coverage.samples[unknown[0]])+[points[j] for j in range(len(points)) if j!=i and active[j]]
            return self.coverage.exact_status(sites)[0]<=999.98
        def prune():
            while True:
                ...
                for gain,i in sorted(gains,reverse=True):
                    if np.all(total_without(i)) or (getattr(self,'hybrid_cover',False) and exact_union_without(i)):
                        active[i]=False
        ...
        if not np.all(union) and not (getattr(self,'hybrid_cover',False) and exact_union_without(-1)):
            raise ArithmeticError('Relocated cover lost coverage')
