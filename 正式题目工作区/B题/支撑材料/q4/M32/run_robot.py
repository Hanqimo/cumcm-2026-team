"""问题四M32 joint正式测试入口；冻结策略已完成30场官方演练。"""
import argparse
...
def main():
    ...
    parser.add_argument('--policy', choices=['joint'], default='joint', help='第四问M32 joint联合可行域策略')
    ...
    if not team:
        ...
        return 2
    if args.check:
        print(f'策略 {args.policy}；第四问M32 joint；已完成30场官方演练。')
    ...
    print('这是已完成30场官方演练的冻结M32 joint策略。')
    print('请在模拟器选择【问题4正式测试】，开始后等待至少5秒，确认“等待机器狗进入”。')
    ...
    (folder / 'manifest.json').write_text(json.dumps(dict(strategy=VERSION, policy=args.policy, policy_parameters=POLICIES[args.policy],
        ...
        config_path=str(config_path), mode='user_selected_P4_formal_not_API_verified'),
