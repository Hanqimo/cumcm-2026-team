...
在该问题中，我们需要基于从第一个检测点获得的信息制定第二个检测点的选择策略。这里的困难在于，选点时并不知道第二次会收到什么反馈，最优定位策略的标准也是不确定的，因此需要先比较各个检测点可能带来的结果。针对本问题的目标，我们在此仅以定位精确度来衡量定位效果，而暂不考虑执行策略消耗的时间差异。

为了比较不同检测结果，本文用“定位损失”衡量检测后还剩下多少位置不确定性。若尚不能在第二检测点原地完成光学精确定位，就以剩余可行区域的最小包围圆半径作为定位损失：半径越小，说明源的位置被限制在越小的范围内，定位效果越好。若整个剩余区域都已位于当前点20米以内，则能够原地完成光学精确定位，定位损失取零。这里的损失是根据可行区域定义的评价指标，并非估计位置与真实位置之间的实际误差。

据此，本文设计了两种评价方法。\textbf{基于贝叶斯更新的期望定位损失模型}先预测不同反馈出现的概率，再计算平均定位损失，选择平均损失最小的位置。\textbf{最小化最坏剩余半径模型}则先找出每个位置可能遇到的最不利反馈，再选择最坏损失最小的位置。两种方法使用相同的定位区域和损失定义，分别回答“平均而言在哪里检测更好”和“在哪里检测能获得更好的最坏情形保证”。

\subsection{准备工作：由观测信息构造定位区域}

设第二个检测点为 \(q\)，它的位置是本问需要决定的量。为简化坐标表示，利用目标圆域的旋转对称性，将首次检测点写成 \(s_1=(a,0)\)，首次示向度记为 \(\beta\)，其中 \(a\ge0\)、\(\beta\in[-\pi,\pi)\) 均由已有观测确定。若在 \(q\) 处得到正常示向度 \(\theta\)，则令 \(s_2=q\)、\(\theta_1=\beta\)、\(\theta_2=\theta\)，并沿用问题一的角域记号 \(W_1,W_2\)。源位置 \(g\) 到两次检测点的距离分别简记为 \(d_1(g)=\|g-s_1\|\)、\(d(g;q)=\|g-q\|\)。
...
在 \(q\) 处进行第二次检测，可能得到强信号、无信号或正常示向度，分别记为 \(\mathrm H\)、\(\mathrm N\) 和 \(\theta\)。收到强信号时，源距 \(q\) 不超过5米；得到正常示向度时，源位于新的测向角域内，且距离大于5米、不超过1500米。

无信号也能提供信息。由于首次能够接收，且两次检测对应同一个固定接收半径，第二次无信号说明源距 \(q\) 比源距 \(s_1\) 更远，同时超过1000米。将这些新信息与 \(K_1\) 取交，得到三种反馈后的剩余区域
...
评价一次检测是否有用，要看得到反馈后还剩下多少位置不确定性。我们还允许机器狗在第二检测点直接使用光学设备：如果整个剩余区域都在当前位置20米以内，就能保证找到源的准确位置，此时定位损失取零；如果尚不能保证原地定位，就用剩余区域的最小包围圆半径作为损失。由此定义
...
损失以米为单位，数值越小，表示这次检测后的定位效果越好。收到强信号时，源已在5米以内，可以原地完成光学定位，所以 \(L_{\mathrm H}=0\)。无信号时源距超过1000米，不能原地定位，所以 \(L_{\mathrm N}=\rho_{\mathrm N}\)。正常示向度则需要按式~\eqref{eq:q2-pair-loss} 检查。

这里要求剩余区域整体位于当前点的20米范围内。即使区域本身很小，只要它离机器狗还很远，也不能当作已经完成定位。这正是同时使用区域半径与最远距离两个量的原因。

\subsection{基于贝叶斯更新的期望定位损失模型}

对于同一个检测点，有的反馈会使区域明显缩小，有的反馈则可能帮助不大。本模型先计算各种反馈有多大可能出现，再按这些概率求平均损失。为此，需要先用首次观测更新对源位置和接收半径的判断，再预测第二次检测的结果。
