...
def certify(q,kind,t):
 ...
 return {'q':q,'kind':kind,'theta':t,'sample_count':len(p),'radius_upper_cpp':r,'sample_diameter_lower':lower,'radius_gap_m':r-lower,'dmax_cpp':geo['dmax'],'dmax_sample':float(np.linalg.norm(p-np.array(q),axis=1).max()),'max_sample_outside_m':float(d.max()-r),'cpp_internal_gap_m':r-geo['radius_lower']}

def main():
 cases=[]
 for q in [tuple(json.loads((ROOT/'results'/name/'best.json').read_text())[k] for k in ['x','y']) for name in ['area','radius']]:
  ...
  cases.append((q,2,0.))
 cases.extend([((500,0),0,0.),((500,0),1,1.57),((500,0),1,.3),((0,500),1,-.5)])
