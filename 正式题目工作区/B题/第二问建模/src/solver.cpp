...
void header(ostream&o){o<<"x,y,J,J_lower,P_H,P_N,P_A,P_loc,E_radius_loss,safe_margin,angle_calls\n";}
void row(ostream&o,Result r){o<<setprecision(15)<<r.q.x<<","<<r.q.y<<","<<r.J<<","<<r.Jlo<<","<<r.ph<<","<<r.pn<<","<<r.pa<<","<<r.p20<<","<<r.rmean<<","<<r.safe<<","<<r.calls<<"\n";}
bool useful(P q){double t=clamp(angle(q),-A,A),r=clamp(dot(q,unit(t)),5.,1500.);return norm(q-unit(t)*r)<=1500+1e-8&&norm(q)>1e-9;}
