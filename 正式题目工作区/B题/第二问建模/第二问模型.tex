...
对 \(k\in\{A,R\}\)，分别令 \(J_{k,*}=\inf_{q\in\mathcal C}J_k(q)\)，第二点选择问题及近优候选区域统一为
\begin{equation}\label{eq:optimization}
\min_{q\in\mathcal C}J_k(q),\qquad
\mathcal C_{k,\eta}=\{q\in\mathcal C:J_k(q)\le(1+\eta)J_{k,*}\},\qquad \eta>0.
\end{equation}
其中 \(\eta\) 是允许的相对目标损失，例如 \(\eta=0.01\) 表示保留对应目标值不超过最优值 \(101\%\) 的候选点。若 \(J_{k,*}=0\)，使用绝对容差 \(\delta>0\) 定义 \(\{q\in\mathcal C:J_k(q)\le\delta\}\)。

还可以用同一预测分布报告原地精确定位的成功概率，定义 \(P_{\rm loc}(q)=P_H(q)+P_N(q)T_N(q)+\int_0^{2\pi}f_\Theta(\theta;q)T_\theta(q)\dd\theta\)。它将各个终止分支的概率相加，反映第二次反馈后能够原地精确定位的可能性。主目标 \(J_A(q)\) 和 \(J_R(q)\) 同时计入未终止分支的剩余不确定性，\(P_{\rm loc}(q)\) 则直接描述达到终止条件的概率。
\end{document}
