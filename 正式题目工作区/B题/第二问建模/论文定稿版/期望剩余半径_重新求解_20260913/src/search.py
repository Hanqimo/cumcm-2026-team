...
def run(a,b):
 ...
 if info['rho1']<=20:
  ev.close()
  (out/'summary.json').write_text(json.dumps({'case':name,'a':a,'beta_deg':b,'status':'initial region already meets completion condition','rho1':info['rho1'],'candidates':[]},indent=2))
  return
 else:
