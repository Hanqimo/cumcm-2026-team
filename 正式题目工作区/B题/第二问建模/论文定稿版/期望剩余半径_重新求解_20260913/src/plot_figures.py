...
report={}
rows=[{k:float(v) for k,v in z.items()} for z in csv.DictReader((ROOT/'results/origin_weights.csv').open())]
representative=next(z for z in rows if z['w']==.5)
...
with plt.rc_context(style):
 ...
 a.text(328,.0155,rf'$P_{{\mathrm{{finish}}}}={100*float(representative["P_finish"]):.2f}\%$',ha='right',va='top',fontsize=9)
