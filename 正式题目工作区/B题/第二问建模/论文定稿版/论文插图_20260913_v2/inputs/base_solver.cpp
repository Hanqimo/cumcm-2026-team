...
struct Eval {P q;Settings set;double Z;int calls=0;double maxgap=0;vector<array<double,7>> profile;
 Val at(double theta){calls++;auto v=feedback(q,1,theta);auto s=boundary(v);double m=mass(v,s,q,1,set.ns,set.nr)/(2*A*Z);if(m<=0)return{};auto c=continuous_mec(s);maxgap=max(maxgap,c.hi-c.lo);bool terminal=norm(farthest(s,q)-q)<=20;return{m,terminal?0:m*PI*c.hi*c.hi,terminal?0:m*PI*c.lo*c.lo,m*terminal,terminal?0:m*c.hi};}
 ...
 Val adaptive(double l,double h,int dep){Val low=quadrature(l,h,set.nt),high=quadrature(l,h,set.nt*2);double err=abs(low.loss-high.loss),tol=set.rel*max(1.0,abs(high.loss));if(dep>=set.depth||(err<=tol&&abs(high.rmean-low.rmean)<=set.rel*max(1.0,abs(high.rmean))&&abs(high.p-low.p)<set.rel*.1&&abs(high.p20-low.p20)<max(2e-6,set.rel*.2)))return high;double m=(l+h)/2;return adaptive(l,m,dep+1)+adaptive(m,h,dep+1);}
};
...
Result evaluate(P q,Settings set){double Z=ZC;Eval e{q,set,Z};Result out;out.q=q;out.safe=0;for(int kind:{0,2}){auto v=feedback(q,kind);auto s=boundary(v);double p=mass(v,s,q,kind,set.ns*2,set.nr*2)/Z;if(p>0){auto c=continuous_mec(s);bool terminal=norm(farthest(s,q)-q)<=20;out.J+=terminal?0:p*PI*c.hi*c.hi;out.Jlo+=terminal?0:p*PI*c.lo*c.lo;out.p20+=p*terminal;out.rmean+=terminal?0:p*c.hi;}if(kind==0)out.ph=p;else out.pn=p;}
...
void header(ostream&o){o<<"x,y,J,J_lower,P_H,P_N,P_A,P_loc,E_radius_loss,safe_margin,angle_calls,x_global,y_global,C1\n";}
void row(ostream&o,Result r){o<<setprecision(15)<<r.q.x<<","<<r.q.y<<","<<r.J<<","<<r.Jlo<<","<<r.ph<<","<<r.pn<<","<<r.pa<<","<<r.p20<<","<<r.rmean<<","<<r.safe<<","<<r.calls<<","<<(SCENARIO_A+cos(SCENARIO_BETA)*r.q.x-sin(SCENARIO_BETA)*r.q.y)<<","<<(sin(SCENARIO_BETA)*r.q.x+cos(SCENARIO_BETA)*r.q.y)<<","<<ZC<<"\n";}
bool useful(P q){if(norm(q)<1e-10)return false;auto sh=boundary(base());if(sh.points.empty())return false;double best=1e99;
 if(feasible(base(),q))return true;
 for(auto p:sh.points)best=min(best,norm(q-p));
 for(auto ar:sh.arcs){double t=pos(angle(q-ar.o));for(int k=-1;k<=2;k++){double v=t+2*PI*k;if(v>=ar.l&&v<=ar.h)best=min(best,abs(norm(q-ar.o)-ar.r));}}
 // The enclosing disk bound is intentionally permissive; false positives only add evaluations.
 return best<=1500+1e-7 || norm(q-TARGET_CENTER)<=3300;
}
int main(int argc,char**argv){if(argc<4)return 1;SCENARIO_A=stod(argv[1]);SCENARIO_BETA=stod(argv[2])*PI/180;TARGET_CENTER={-SCENARIO_A*cos(SCENARIO_BETA),SCENARIO_A*sin(SCENARIO_BETA)};argc-=2;argv+=2;ZC=normalizer();if(!(ZC>0)){cerr<<"Incompatible observation: C1 <= 0\n";return 2;}string mode=argv[1];if(mode=="info"){auto sh=boundary(base());auto c=continuous_mec(sh);cout<<setprecision(15)<<"{\"C1\":"<<ZC<<",\"rho1\":"<<c.hi<<",\"center\":["<<c.c.x<<","<<c.c.y<<"],\"points\":[";for(size_t i=0;i<sh.points.size();i++){if(i)cout<<",";cout<<"["<<sh.points[i].x<<","<<sh.points[i].y<<"]";}cout<<"]}\n";}
 else if(mode=="eval"){P q={stod(argv[2]),stod(argv[3])};int level=argc>4?stoi(argv[4]):2;PRIOR=argc>5?stoi(argv[5]):0;header(cout);row(cout,evaluate(q,settings(level)));}
 ...
 else if(mode=="geometry"){P q={stod(argv[2]),stod(argv[3])};int k=stoi(argv[4]);double t=stod(argv[5]);auto v=feedback(q,k,t);auto s=boundary(v);auto c=continuous_mec(s);cout<<setprecision(15)<<"{\"radius_lower\":"<<c.lo<<",\"radius_upper\":"<<c.hi<<",\"dmax\":"<<(s.points.empty()?0:norm(farthest(s,q)-q))<<",\"center\":["<<c.c.x<<","<<c.c.y<<"],\"points\":[";for(size_t i=0;i<s.points.size();i++){if(i)cout<<",";cout<<"["<<s.points[i].x<<","<<s.points[i].y<<"]";}cout<<"],\"arcs\":[";for(size_t i=0;i<s.arcs.size();i++){auto a=s.arcs[i];if(i)cout<<",";cout<<"["<<a.o.x<<","<<a.o.y<<","<<a.r<<","<<a.l<<","<<a.h<<"]";}cout<<"]}\n";}

 else if(mode=="branches"){P q={stod(argv[2]),stod(argv[3])};double z=normalizer();cout<<"kind,probability,radius_lower,radius_upper,center_x,center_y,loss_contribution\n";for(int kind:{0,2}){auto v=feedback(q,kind);auto sh=boundary(v);double p=mass(v,sh,q,kind,20,24)/z;auto c=continuous_mec(sh);cout<<setprecision(15)<<kind<<","<<p<<","<<c.lo<<","<<c.hi<<","<<c.c.x<<","<<c.c.y<<","<<(norm(farthest(sh,q)-q)<=20+1e-8?0:p*PI*c.hi*c.hi)<<"\n";}}
 else if(mode=="mc"){P q={stod(argv[2]),stod(argv[3])};int n=stoi(argv[4]);unsigned seed=argc>5?stoi(argv[5]):812371;mt19937_64 gen(seed);uniform_real_distribution<double> U(0,1);auto regN=feedback(q,2),regH=feedback(q,0);auto cn=continuous_mec(boundary(regN)),ch=continuous_mec(boundary(regH));double sum=0,sum2=0,maxres=-1e99,sumr=0,sumr2=0;int nh=0,nn=0,n20=0,invalid=0;vector<double> radii;radii.reserve(n);for(int i=0;i<n;i++){double r,phi,R;do{r=sqrt(25+(2250000-25)*U(gen));R=1000+500*U(gen);phi=(2*U(gen)-1)*A;}while(r>R||norm(unit(phi)*r-TARGET_CENTER)>1800);P g=unit(phi)*r;double d=norm(g-q);MEC c;Region v;if(d<=5){nh++;c=ch;v=regH;}else if(d>R){nn++;c=cn;v=regN;}else{double theta=angle(g-q)+(2*U(gen)-1)*A;v=feedback(q,1,theta);auto sh=boundary(v);if(sh.points.empty())invalid++;c=continuous_mec(sh);}if(!feasible(v,g,1e-6))invalid++;maxres=max(maxres,norm(g-c.c)-c.hi);auto sh=boundary(v);bool terminal=norm(farthest(sh,q)-q)<=20;double lossr=terminal?0:c.hi,l=PI*lossr*lossr;sum+=l;sum2+=l*l;sumr+=lossr;sumr2+=lossr*lossr;n20+=terminal;radii.push_back(lossr);}sort(radii.begin(),radii.end());double mean=sum/n,se=sqrt(max(0.,sum2/n-mean*mean)/(n-1));cout<<setprecision(15)<<"{\"n\":"<<n<<",\"seed\":"<<seed<<",\"mean_area\":"<<mean<<",\"standard_error\":"<<se<<",\"P_H\":"<<(double)nh/n<<",\"P_N\":"<<(double)nn/n<<",\"P_loc\":"<<(double)n20/n<<",\"mean_radius\":"<<sumr/n<<",\"radius_standard_error\":"<<sqrt(max(0.,sumr2/n-sumr*sumr/(double(n)*n))/(n-1))<<",\"radius_median\":"<<radii[n/2]<<",\"radius_p90\":"<<radii[(int)(n*.9)]<<",\"radius_p95\":"<<radii[(int)(n*.95)]<<",\"radius_p99\":"<<radii[(int)(n*.99)]<<",\"max_truth_coverage_residual\":"<<maxres<<",\"invalid\":"<<invalid<<"}\n";}
