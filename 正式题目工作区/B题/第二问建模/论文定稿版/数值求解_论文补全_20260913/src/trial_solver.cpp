...
struct TrialEvaluator {
 ...
 TrialValue at(double theta){
  ...
  return{m,optical?0:m*c.hi,m*(c.hi<=20.),m*optical};
 }
...
void trial_evaluate(P q,int level,int scan){
 ...
 for(int kind:{0,2}){auto v=feedback(q,kind);auto s=boundary(v);double p=mass(v,s,q,kind,st.ns*2,st.nr*2)/ZC;if(kind==0)ph=p;else pn=p;
  ...
  total=total+TrialValue{p,optical?0:p*c.hi,p*(c.hi<=20.),p*optical};if(kind==2)fn=p*(c.hi<=20.);
 }
...
void trial_mc(P q,int n,int seed){
 ...
 for(int i=0;i<n;i++){
  ...
  bool optical=norm(farthest(s,q)-q)<=20.;double j=optical?0:c.hi;sumj+=j;sumj2+=j*j;done+=c.hi<=20.;onsite+=optical;maxres=max(maxres,norm(g-c.c)-c.hi);
 }
