...
每次反馈先转化为源位置约束，再与已有信息取交。利用旋转对称性，取 \(s_1=(a,0)\)、\(\theta_1=\beta\)，其中 \(a\ge0\)、\(\beta\in[-\pi,\pi)\) 为给定参数。考察第二点 \(q\) 及其可能读数 \(\theta\) 时，令 \(s_2=q\)、\(\theta_2=\theta\)，并简记 \(d_1(g)=\|g-s_1\|\)、\(d_2(g;q)=\|g-q\|\)。

首次正常示向度同时给出方向、有效接收和非强信号三类信息，故源位置可行区域为 \(K_1=\{g\in\Omega\cap\mathcal W_1:r_{\mathrm{strong}}<d_1(g)\le1500\}\)，并要求 \(K_1\ne\varnothing\)。对任一 \(g\in K_1\)，接收半径还须满足 \(\max\{1000,d_1(g)\}\le R\le1500\)，第二次检测继续使用同一个 \(R\)。

以 \(\mathsf H\)、\(\mathsf N\) 和 \(\theta\in[0,2\pi)\) 表示强信号、无信号和正常示向度反馈。无信号要求首次能够接收、第二次不能接收，即存在 \(R\) 满足 \(\max\{1000,d_1(g)\}\le R<d_2(g;q)\)。消去共享的 \(R\)，三类反馈下的源位置可行区域统一记为
\begin{equation}
\begin{aligned}
K_{\mathsf H}(q)&=\{g\in K_1:d_2(g;q)\le r_{\mathrm{strong}}\},\\
K_{\mathsf N}(q)&=\{g\in K_1:d_2(g;q)>\max\{1000,d_1(g)\}\},\\
K_\theta(q)&=\{g\in K_1\cap\mathcal W_2:r_{\mathrm{strong}}<d_2(g;q)\le1500\}.
\end{aligned}
...
上述区域保留题目的严格距离条件，未必包含全部边界点。对任一非空有界区域 \(K\)，统一定义 \(d_{\max}(q;K)=\sup_{g\in K}\|g-q\|\)、\(\rho(K)=\min_{c\in\mathbb R^2}d_{\max}(c;K)\)。连续距离函数在区域及其闭包上的上确界相同，因此这样即可计算最小包围圆，无需另设一套闭包区域记号。为简化后文，记 \(\rho_1=\rho(K_1)\)、\(\rho_z(q)=\rho(K_z(q))\)。
...
\end{equation}
前两项为离散反馈概率，最后一项为正常角度的预测密度分量，其在 \([0,2\pi)\) 上的积分等于正常反馈发生的概率，三者合计为1。至此，反馈概率与相应区域半径均已确定，可以构造选点指标。
...
\end{equation}
第一项衡量无信号的预期代价，第二项衡量正常反馈的平均定位损失，强信号分支以零损失参与加权。预测密度为零的反馈不影响期望；若相应区域为空，积分中的该项按零计，不另定义空集的包围圆。给定 \((a,\beta)\) 与 \(\lambda\)，求 \(\inf_{q\in\mathcal Q}J_{\mathrm E}(q)\)，最小值可达时取 \(q_{\mathrm E}^*\in\arg\min_{q\in\mathcal Q}J_{\mathrm E}(q)\)。数值上，对候选点计算反馈权重和区域半径，经角度积分得到评分，再通过空间搜索与局部加密寻找近优点。
...
\end{equation}
最小值可达时，取 \(q_{\mathrm W}^*\in\arg\min_{q\in\mathcal Q}J_{\mathrm W}(q)\)。内层取最坏值时保留所有可实现反馈，包括边界状态及退化交集；外层据此寻找最坏损失较小的检测点。数值求解采用两层搜索，内层细分正常角度区间并与强信号、无信号分支比较，外层在检测点空间逐步加密，同时检查原地定位条件。
