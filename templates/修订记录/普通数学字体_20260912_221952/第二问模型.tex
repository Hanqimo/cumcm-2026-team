...
% 页面、行距与段落。
\usepackage[a4paper,top=2.3cm,bottom=2.3cm,left=2.3cm,right=2.3cm,footskip=1.2cm]{geometry}
\usepackage{setspace}
\singlespacing
...
正常示向度对应的正向角域记为 \(\mathcal W_i=\{s_i+t(\cos(\theta_i+\delta),\sin(\theta_i+\delta)):t\ge0,\ |\delta|\le\alpha\}\)，\(\mathbf1_A\) 表示集合 \(A\) 的指示函数。正文与论文的问题二章节保持一致，所需其余记号随推导定义。
