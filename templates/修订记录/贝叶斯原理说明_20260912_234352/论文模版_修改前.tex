...
题面没有给出具体概率分布及空间相关关系。本模型对未知源位置 \(G\) 采用 \(\Omega\) 内按面积均匀的先验，对固定接收半径采用 \(R\sim\operatorname{U}[1000,1500]\)，对两个不同检测点的误差采用 \(E_1,E_2\sim\operatorname{U}[-\alpha,\alpha]\)，并将 \(G,R,E_1,E_2\) 在观测前相互独立作为简化假设。不同地点误差的独立性不能仅由地点不同推出；若实际误差存在明显空间相关性，应相应修改联合分布及反馈预测概率。
...
\end{equation}
这里 \(C_1\) 保证密度积分为1。该更新的直观含义是，较远的位置需要较大的接收半径才能解释首次接收，因而在更新后获得较低权重。进一步，对于正后验密度的位置，有 \(R\mid G=g,I_1\sim\operatorname{Unif}[\max\{1000,d_1(g)\},1500]\)；位置与半径在后验中由此产生关联，第二次预测须保留这一关系。
