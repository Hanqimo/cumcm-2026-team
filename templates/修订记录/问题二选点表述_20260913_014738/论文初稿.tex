...
进一步构造60组随机观测，每组包含3至8个检测点，检测距离为150至1450米，读数由真实方向叠加 \([-1^\circ,1^\circ]\) 内的误差生成。每组均保留真实源，并通过两种独立途径核验：用线性规划计算区域在给定方向上的最远投影，与顶点投影的最大值比较；另用凸优化直接求最小覆盖半径，与支撑点枚举结果比较。两项最大差分别约为 \(4.55\times10^{-13}\) 米和 \(5.95\times10^{-10}\) 米。上述结果支持几何算法的数值一致性，这些差值是不同计算方法之间的差异，并非设备定位精度。
