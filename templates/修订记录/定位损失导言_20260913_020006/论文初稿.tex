...
第一次检测后，我们已经知道干扰源可能位于哪些位置，但还不能确定它的准确位置。第二问需要决定下一次去哪里检测，并给出一片定位效果较好的候选区域。困难在于，选点时还不知道第二次会收到什么反馈，因此不能只挑一个读数分析，而要先比较各个检测点可能带来的结果。

本文采用两种评价方法。\textbf{基于贝叶斯更新的期望定位损失模型}先预测不同反馈出现的概率，再计算平均定位损失，选择平均损失最小的位置。\textbf{最小化最坏剩余半径模型}则先找出每个位置可能遇到的最不利反馈，再选择最坏损失最小的位置。两种方法使用相同的定位区域和损失定义，分别回答“平均而言在哪里检测更好”和“在哪里检测能获得更好的最坏情形保证”。
