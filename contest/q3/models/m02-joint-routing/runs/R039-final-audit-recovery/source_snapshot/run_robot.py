...
def main():
    ...
    parser.add_argument('--policy', choices=['route_guard','region_coupled','continuous_cover','combined_cover'], default='combined_cover', help='选择冻结策略，默认组合覆盖策略')
    ...
    try:
        ...
        return 2
    paths = [args.config] if args.config else [ROOT / 'config.json', ROOT.parent / 'simple-strategy' / 'config.json']
    ...
    snapshot.mkdir()
    hashes = {}
    for p in list(ROOT.glob('*.py')) + [ROOT / 'policies.json']:
    ...
    (folder / 'manifest.json').write_text(json.dumps(dict(strategy=VERSION, policy=args.policy, policy_parameters=POLICIES[args.policy],
        python=sys.version, numpy=numpy.__version__, scipy=scipy.__version__, source_sha256=hashes,
        config_path=str(config_path), mode='user_selected_P3_practice_not_API_verified'),
    ...
    try:
        ...
        summary = dict(strategy=VERSION, policy=args.policy, policy_parameters=POLICIES[args.policy], outcome=outcome, error=error_text,
            ...
            note='实际目标总数和正式成绩以模拟器为准；完整性证书以问题3全向规则为前提。')
