...
def geometry_checks():
    ...
    return dict(squares=len(coverage.centers), all_seven_certified=True,
                one_station_rejected=True, triangle_radius=triangle_radius)
...
def main():
    ...
    scenarios = [(seed, 'sin', False) for seed in range(args.seed_start, args.seed_start + args.seeds)]
    if args.extended:
    ...
    for index, (seed, mode, edge) in enumerate(scenarios):
        ...
        if args.extended and index < 10:
            variants += ['opportunistic_on', 'channel_order']
            ...
            try:
                with contextlib.redirect_stdout(io.StringIO()):
                    if variant == 'baseline':
                        ...
                        planner = Planner(world, opportunistic=variant == 'opportunistic_on',
