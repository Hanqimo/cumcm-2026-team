...
上述假设用于构造第二问的反馈概率并比较平均定位效果。第一问的几何判据，以及第三问的保证清除与搜索终止判据，不依赖这些概率假设。
...
问题二研究如何依据首次示向度选择第二个检测点，并给出可供选择的区域。由于第二次反馈尚未出现，需要先预测各种结果，再比较检测点的平均定位效果。本文用剩余区域的最小包围圆半径衡量位置不确定性；若能在检测点原地完成光学精确定位，则不再保留位置不确定性。在此基础上，通过贝叶斯更新计算各种反馈的概率，寻找平均定位效果最好的位置，并将效果接近最优的位置组成候选区域。
\subsection{问题三分析}
...
在此基础上，本文建立\textbf{基于贝叶斯更新的期望定位损失模型}。先根据首次观测更新对源位置和接收半径的判断，再预测各个检测点可能收到的反馈及其概率，计算平均定位损失，并选择平均损失最小的位置。进一步，将平均损失与最优值相差不大的位置保留为候选区域，供实际选点使用。
...
对于同一个检测点，有的反馈会使源位置可行区域明显缩小，有的反馈则可能帮助不大。本模型先计算各种反馈有多大可能出现，再按这些概率求平均损失。为此，需要先用首次观测更新对源位置和接收半径的判断，再预测第二次检测的结果。
...
至此，对于每个待选位置 \(q\)，我们既知道不同反馈会留下多大的定位损失，也知道这些反馈出现的概率。以 \(Z\) 表示尚未得到的第二次反馈，将损失按预测概率求平均，记为 \(J(q)\)。第二检测点的决策就是在搜索范围 \(Q\) 内，使这个平均损失尽可能小，即

\begin{equation}
 \min_{q\in Q}\quad J(q)=\mathbb E[L_Z(q)\mid I_1,q]
...
题目还要求给出候选区域，而不只是一个最优点。为此，记最优期望损失为 \(J^*=\inf_{q\in Q}J(q)\)，并保留所有期望损失与它相差不大的位置。若允许比最优值多出 \(\epsilon\) 米的期望损失，则候选区域为

\begin{equation}
 Q_\epsilon=\{q\in Q:J(q)\le J^*+\epsilon\}.
...
容差 \(\epsilon\) 表示愿意让期望定位损失增加多少米，以获得更多可选位置。本文暂以1米为默认值，后续计算将用0.5米、2米进行对照。它是可以调整的策略参数，需要通过后续计算比较区域大小和移动距离后判断是否合适。采用绝对容差，也使最优损失为零时仍能保留附近效果相近的位置。

数值求解时，先在 \(Q\) 内设置网格并计算各点的 \(J\)，找到当前最低损失，再按容差筛选候选位置。对损失较低的区域和接近筛选边界的位置加密网格，并提高反馈概率积分的精度。不能因为网格的几个顶点合格，就认定整个网格都合格；边界附近尚不能确认的位置应继续检查，最终推荐点也要单独复算。

结果图应标出首次检测点、源位置可行区域 \(K_1\)、检测点候选区域 \(Q_\epsilon\) 及推荐点，同时注明损失基准和容差。候选区域随首次观测 \((a,\beta)\) 重新计算。若只完成了有限精度的搜索，就将结果称为数值近似候选区域；网格加密后结果趋于稳定，可以支持数值结果的可信度，但本身不能证明已经找到连续空间中的全局最优值。

\subsubsection{从候选区域中确定实际检测点}

候选区域内的各点都满足期望损失的容差要求，因此可以进一步考虑移动是否方便。本文在这些位置中选择离首次检测点最近的一点；在最小值能够取到时，实际推荐点满足

\begin{equation}
 q^*\in\arg\min_{q\in Q_\epsilon}\|q-s_1\|.
 \label{eq:q2-candidate-selection}
\end{equation}

这一选择是在期望损失不超过 \(J^*+\epsilon\) 的前提下尽量减少移动。实际推荐点不一定取得最小期望损失，但平均定位效果仍接近最优。若多个点距离相同，则优先选择期望损失较小者；计算时只从已经复核的候选点中选择。
