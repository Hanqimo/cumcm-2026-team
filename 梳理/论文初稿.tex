...
\usepackage{graphicx}
\graphicspath{{./}{../正式题目工作区/B题/第二问建模/论文定稿版/期望剩余半径_重新求解_20260913/figures/}}
...
\newcommand{\paperabstract}{本文研究无线电干扰源的快速自动定位与清除，针对示向度误差有界、同一地点误差固定的特点，采用集合可行域刻画定位不确定性。针对问题一，将每次示向度观测表示为以检测点为顶点的正向角域，并用方向向量叉积写成两个半平面的交集，多次观测取交得到凸定位区域；证明有界区域的直径等于最远顶点对距离，并构造一个由合法测向角域生成的边长 38 米的等边三角形，其最小包围圆半径约 21.94 米、大于直径圆半径 19 米，说明以定位区域直径为直径的圆不一定能覆盖该区域；进而由 20 米光学清除半径导出保证清除的充要条件为最小包围圆半径不超过 20 米。针对问题二、三、四，本文分别研究第二个检测点的主动选择、未知数量多源干扰源的在线搜索与清除，以及含定向源情形下的检测清除策略。}
...
\end{equation}
其中，\(\mathbf1_{\{\cdot\}}\) 在条件满足时取1，否则取0。强信号一定满足完成条件；无信号和正常示向度则都要检查剩余区域能否被20米圆覆盖。无信号也可能使剩余区域满足这一条件。
...
针对问题一，本文对几何算法进行了独立交叉核验。在跨越 \(0^\circ/360^\circ\) 及坐标轴附近的 3500 个测试点上，直接角度判断与半平面不等式判断的结果一致；对 60 组随机多站观测，用线性规划计算区域在给定方向的最远投影并与顶点投影比较，用凸优化直接求最小覆盖半径并与支撑点枚举比较，两项最大差分别约 \(4.55\times10^{-13}\) 米和 \(5.95\times10^{-10}\) 米；对 200 组凸多边形，圆弧与交点枚举所得最近清除位置与独立凸优化结果的最大移动距离差约 \(3.25\times10^{-7}\) 米，且全部满足 19.5 米的工程清除距离约束。此外还核验了空集、无界、点、线段等退化情形的处理以及平移旋转不变性。
...
本参赛队在竞赛过程中使用了AI工具，主要用于文献检索、数值计算代码的编写与调试、论文排版与符号统一，详细使用情况见支撑材料。
