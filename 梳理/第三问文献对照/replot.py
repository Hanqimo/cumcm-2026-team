...
for ax,key,row,color,title in zip(axs,['baseline','v6'],[a[i],b[i]],[colors[0],colors[2]],['(a) 谨慎测向：逐源处理','(b) 全程协同：联合安排任务']):
...
fig.subplots_adjust(left=.105,right=.975,bottom=.28,top=.85,wspace=.52)
...
ax.set(yticks=range(3),yticklabels=['谨慎测向','局部协同','全程协同'],xlabel='平均分项时间 / (s·源$^{-1}$)',xlim=(0,380),ylim=(2.55,-.65))
ax.set_title('(b) 分项时间比较',loc='left',pad=13);ax.xaxis.grid(alpha=.15,lw=.5)
