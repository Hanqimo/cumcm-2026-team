...
沿用第三问的区域 $\Omega$、位置域 $K$、最小包围圆中心 $c(K)$ 和半径 $\rho(K)$。设源位置为 $x$，作用半径 $R\in[1000,1500]$ m，类型 $z=0,1$ 分别表示全向与定向，单位向量 $n$ 表示发射朝向。机器人在 $q$ 处的可接收状态为
...
为使改进具有可检验的起点，构造文献启发的谨慎贪心适配版，记为 CG。用位置域凸包的均匀面积均值 $\mu$ 和协方差近似原论文的高斯估计，令 $\sigma_{\max}$ 为最大特征值的平方根，$v$ 为对应特征向量的垂直方向。按照文献[2]的 Algorithm 1，在 $\mu\pm av$ 中选择离当前位置较近者，其中

$$
a=\frac{\sigma_{\max}}{\sqrt{\sigma_\beta^2-\sigma_s^2}},\qquad
 \sigma_\beta=\frac{\pi}{2\Phi^{-1}(1-\beta/2)}.
\tag{2}
$$

测试前固定 $\beta=0.05$，$\sigma_s^2$ 取 $\pm1^\circ$ 均匀误差的二阶矩。位置域由有界几何约束更新，清除用包围圆判断，不把协方差当作最坏误差保证。若无信号使域不变且两点均已测量，则以 $30^\circ$ 步长旋转同半径候选以避免重复测量；无可用点时转有限光学覆盖。此为公开说明适配步骤的基线，不是原论文完整 EKF 算法的复现。
...
$$
L_B=\max\{1000,\max_{p\in P}(\|p-c\|-r)\},\qquad
...
对有效接收点取 $d=p-c$，对被迫背向点取 $d=c-q$。当 $\|d\|>r$ 时，块内位置变化引起的向量方向偏移至多为 $\arcsin(r/\|d\|)$；因此相容朝向必在以 $\arg d$ 为中心、半宽为 $\pi/2+\arcsin(r/\|d\|)$ 的闭圆周区间 $A(d,r)$ 内。当 $\|d\|\le r$ 时不加入该项限制。若 $Q_B\ne\varnothing$ 且
...
表 2　文献适配基线、简洁旧版与本文策略的官方演练比较（时间单位：s/源）
...
[1] Tokekar P, Isler V. Sensor placement and selection for bearing sensors with bounded uncertainty[C]//IEEE ICRA. 2013. DOI: 10.1109/ICRA.2013.6630920.
